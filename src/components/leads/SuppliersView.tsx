...
import { SUPPLIERS, getProjectsForSupplier, STATES } from "@/data/states";
    ...
    >
      <div className="space-y-6">
        {SUPPLIERS.map((sup) => {
          ...
          const active = subs.filter((s) => s.state !== "finance" || s.state !== "paid");
          return (
            <section key={sup.id}>
              ...
              {active.length === 0 ? (
                ...
                <div className="space-y-2">
                  {active.map((s) => {
                    const stateInfo = STATES.flatMap((p) => p.states.map((st) => ({ ...st, stageTitle: p.title }))).find((x) => x.id === s.state);
                    return (
                      <button
                        key={s.id}
                        ...
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            ...
                            <span className="text-[10px] uppercase tracking-wider text-muted-foreground">
                              {stateInfo?.stageTitle} · {stateInfo?.title}
                            </span>
                          </div>
                        </div>
                      </button>
