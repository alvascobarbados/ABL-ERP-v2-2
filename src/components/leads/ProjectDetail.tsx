...
// ─────────── Log section (immutable audit trail) ───────────
const LOG_DOT_COLOR: Record<ProjectLogActionType, string> = {
  stage_change: "#1B2A4E",
  field_edit: "#888888",
  flag_toggle: "#E97B2C",
  note_added: "#888888",
  project_created: "#888888",
  archive: "#888888",
  unarchive: "#888888",
  trash: "#888888",
  restore: "#888888",
  mark_paid: "#6B8E5A",
  line_item_change: "#888888",
};

function dotColorFor(e: ProjectLogEntry): string {
  if (e.actionType === "stage_change") {
    const target = e.metadata?.toPipeline;
    if (target === "design") return "#8B3A62";
    if (target === "shipping") return "#1D9E75";
    return "#1B2A4E";
  }
  return LOG_DOT_COLOR[e.actionType] ?? "#888888";
}
...
function dateGroupKey(d: Date): string {
  ...
  const long = `${dd.toLocaleString("en-US", { weekday: "short" })} ${dd.getDate()} ${dd.toLocaleString("en-US", { month: "short" })}`.toUpperCase();
  if (diff === 0) return `TODAY · ${long}`;
  if (diff === 1) return `YESTERDAY · ${long}`;
  return long;
}

const INITIAL_LOG_LIMIT = 8;
...
const LogSection = ({ entries }: { entries: ProjectLogEntry[] }) => {
  ...
  const visible = expanded ? sorted : sorted.slice(0, INITIAL_LOG_LIMIT);
  const remaining = sorted.length - visible.length;

  // Group by date key while preserving newest-first order.
  const groups: { key: string; items: ProjectLogEntry[] }[] = [];
  for (const e of visible) {
    const key = dateGroupKey(e.ts);
  ...
  return (
    <section className="px-3 sm:px-5 pt-6 pb-2 border-b border-border/60">
      <h2 className="px-3 text-[10px] uppercase tracking-[0.22em] text-muted-foreground font-medium mb-1.5">Log</h2>
      {sorted.length === 0 ? (
        <div className="px-3 py-3 text-[13px] text-muted-foreground italic">No activity yet</div>
      ) : (
        <div className="px-3">
          {groups.map((g, gi) => (
            <div
              key={g.key + gi}
              className={cn(gi > 0 && "mt-3 pt-3 border-t border-dashed")}
              style={gi > 0 ? { borderColor: "hsl(var(--brand-navy) / 0.15)" } : undefined}
            >
              <div className="text-[10px] uppercase tracking-[0.18em] font-medium text-muted-foreground/80 mb-1.5">
                {g.key}
              </div>
              <ul className="space-y-2">
                {g.items.map((e) => {
                  const t = e.ts.toLocaleString("en-US", { hour: "numeric", minute: "2-digit" });
                  const parts = e.description.split(e.actor.displayName);
                  return (
                    <li key={e.id} className="flex items-start gap-2.5">
                      <span
                        className="mt-1.5 inline-block rounded-full shrink-0"
                        style={{ width: 7, height: 7, backgroundColor: dotColorFor(e) }}
                      />
                      <div className="flex-1 min-w-0">
                        <div className="text-[13px] leading-snug text-foreground">
                          {parts.length > 1 ? (
                            <>
                              <span className="font-semibold" style={{ color: "hsl(var(--brand-navy))" }}>
                                {e.actor.displayName}
                              </span>
                              {parts.slice(1).join(e.actor.displayName)}
                            </>
                          ) : (
                            e.description
                          )}
                        </div>
                        <div className="text-[11px] text-muted-foreground tabular mt-0.5">{t}</div>
                      </div>
                    </li>
                  );
                })}
          ...
          {expanded && sorted.length > INITIAL_LOG_LIMIT && (
            <button
              onClick={() => setExpanded(false)}
              className="mt-3 ml-3 text-[12px] font-medium text-muted-foreground hover:underline"
            >
              Show less
            </button>
          )}
